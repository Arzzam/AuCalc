...
function Select(props) {
    ...
    function scrollOptions(increment) {
        if (contentRef.current) {
            ...
            increment = increment < 0 ? (contentChildren.length + increment) : increment;
    ...
    return (
        ...
        >
            <div
                className="relative text-left flex flex-wrap items-center border-solid border border-slate-300 group-hover:border-slate-500 group-focus:border-slate-500 rounded cursor-pointer justify-between min-h-fit transition-all"
            >
                <div
                    className="px-2"
                >
                ...
                <div
                    className="p-2 text-slate-300 group-hover:text-slate-500 group-focus:text-slate-500"
                >
                    <svg height="20" width="20" viewBox="0 0 20 20" focusable="false" fill="currentcolor" stroke="currentcolor">
                ...
                {isMenuOpen &&
                    ...
                    >
                        {Object.keys(props.options).map((key, index) => {
                            return (
                                <div
                                    className={`mx-1 px-2${key === selectedOption ? ' bg-black text-white': ''} rounded`}
                                    key={index}
